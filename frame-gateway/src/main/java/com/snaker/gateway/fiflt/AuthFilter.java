package com.snaker.gateway.fiflt;
...
public class AuthFilter implements GlobalFilter, Ordered {
    ...
    private static final String[] whiteList = {"/auth/login", "/user/register", "/system/v2/api-docs",
            "/auth/captcha/check", "/auth/captcha/get", "/auth/login/slide", "/system/file/ftp/download","/dfs/file/getPicture",
            "/system/file/showFileByResponse","/models/newModel","/system/services/oaCallService"};
//    private static  final  String[] likeWhiteList = {"models/newModel","models/editModel","download","modeler.html"};
    private static  final  String[] likeWhiteList = {"models/newModel","models/editModel","modeler.html"};
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain){
        ...
        ServerHttpRequest mutableReq = exchange.getRequest().mutate().header(Constants.CURRENT_ID, userId)
                .header(Constants.CURRENT_LOGINNAME, jo.getString("loginName"))
                .header(Constants.CURRENT_USERNAME, URLEncoder.encode(jo.getString("name"), "UTF-8")).build();
