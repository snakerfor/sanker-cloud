package com.snaker.common.redis.aspect;

import com.snaker.common.redis.annotation.RedisCache;
import com.snaker.common.redis.annotation.RedisEvict;
...
public class RedisAspect {
     ...
     */
    @Pointcut("@annotation(com.snaker.common.redis.annotation.RedisCache)")
    public void redisCachePoint() {
    }

    @Pointcut("@annotation(com.snaker.common.redis.annotation.RedisEvict)")
    ...
    public void evict(JoinPoint point) {
        ...
        logger.info("[Redis切面==>>> 清除rediskey:{}]", rk);
        redis.delete(rk);
    }
    ...
    public Object WriteReadFromRedis(ProceedingJoinPoint point) {
        try {
            ...
            if (redisCache != null && redisCache.read()) {
                // 查询操作
                logger.info("[RedisCache切面==>>> 由方法:{}进入]", method.getName());
                ...
                if (obj == null) {
                    ...
                    logger.info("[RedisCache切面==>>> Redis中不存在该记录,从数据库查找]");
                    obj = point.proceed();
                    if (obj != null) {
                    ...
                    logger.info("[RedisCache切面==>>> Redis有该记录]");
                }
                return obj;
            }
        } catch (Throwable ex) {
            logger.error("[RedisCache切面执行异常:{}]", ex);
        }
        return null;
    }
